...
int previoEstadoEstable = -1; // Estado ESTABLE previo
...
void wunder() {
  ...
  if (mySerial.available()){
    ...
    if (wunderLeerTrama){
      ...
      ControlFases();
      if (trama2 != trama) { 
        Serial.print(trama); Serial.print("  523 - Fase: "); Serial.print(fase); Serial.print("  Estado: "); Serial.print(estado); Serial.print("  Peso: "); Serial.print(peso2); Serial.print(" Tara: "); Serial.print(tara); Serial.println("  ");}
      //minim*.95
      //Serial.println(trama);
    ...
    switch (fase) 
      {
        case 0: //   FASE0 -  NO TARA 
          ...
          if ( tara == 1 ) { 
            ...
            Serial.print(trama); Serial.print("  609 - Fase: "); Serial.print(fase); Serial.print("  Estado: "); Serial.print(estado); Serial.print("  Peso: "); Serial.print(peso2); Serial.print("  Tara: "); Serial.print(tara); Serial.println("  "); 
            //ComprobarConfeccion (); //COMPROBAMOS SI SE HA MODIFICADO LA CONFECCION} 
            } //PASAMOS A FASE 1
            else {PintaPantalla();
            //Serial.print(trama);  Serial.print("  Tara: "); Serial.print(tara); Serial.println("  ");
            }
          break;
        ...
        case 1: //  FASE1 - TARA HECHA - ESTAMOS EN ESPERA DE PESO
          ...
          if ( peso2 > 0 ) {
             ...
             fase = 2 ;
              ...
              Serial.print(trama); Serial.print("  623 - Fase: "); Serial.print(fase); Serial.print("  Estado: "); Serial.print(estado); Serial.print("  Peso: "); Serial.print(peso2); Serial.println("  "); 
              PintaPantalla();
              }
        ...
        case 2://   FASE2 - TARA HECHA Y PESO DISTINTO DE CERO - ESTAMOS PESANDO
          ...
          if (estable==1 ) {
            if (PesoEnRangoPosible()) {
              ...
              Serial.print(trama); Serial.print("  640 - Fase: "); Serial.print(fase); Serial.print("  Estado: "); Serial.print(estado); Serial.print("  Peso: "); Serial.print(peso2); Serial.println("  "); 
              }
              else {
                if (peso2 ==0) {
                  ...
                  estado= -1;
                  Serial.println("--------------Cambio a Fase 1");
                  Serial.print(trama); Serial.print("  649 - Fase: "); Serial.print(fase); Serial.print("  Estado: "); Serial.print(estado); Serial.print("  Peso: "); Serial.print(peso2); Serial.println("  ");
                }
              }
            }else {  //NO ES ESTABLE
        ...
        case 3: // FASE3  PESO ESTABLE Y DENTRO DE RANGOPOSIBLE - TENEMOS PESO ESTABBLE
          ...
          if (estable==1)  { //SI EL PESO ES ESTABLE
            if (PesoEnRangoPosible()) {// Y ESTA DENTRO DEL RANGO
                  // ANALIZAMOS SI HAY CAMBIO DE ESTADO ESTABLE
                  //GUARDAMOS ESTADO Y PESO ESTABLE PREVIO
                  previoEstadoEstable =  estadoEstable;
                  previoPesoEstable =  pesoEstable;
                  //Calculamos Estado y Peso Estable
                  pesoEstable = peso2;
                  estadoEstable= CalculaEstado(peso2);
                  if (estado != estadoEstable){
                    estado = estadoEstable;
                    Serial.println("------------------------------------------------------Cambio de estado dentro de Fase 3 con peso estable: ");
                    Serial.print(trama); Serial.print(" 681 - Fase: "); Serial.print(fase); Serial.print("  Estado: "); Serial.print(estado); Serial.print("  Peso: "); Serial.print(peso2); Serial.println("  ");
                    
                  }
            }else{
              // EL PESO ES ESTABLE PERO NO ESTA DENTRO DEL RANGO
              //SI SE ESTABILIZA EL PESO FUERA DE RANGOPOSIBLE VUELVE A LA FASE 2
                  if (peso2 != 0) {
                    pesoEstable = 0; estadoEstable = 0;
                    estado= CalculaEstado(peso2);
                    fase = 2;
                    Serial.println("------------------------------------------------------Volvemos a Fase 2 por peso estable fuera de rango");
                    Serial.print(trama); Serial.print(" 688 - Fase: "); Serial.print(fase); Serial.print("  Estado: "); Serial.print(estado); Serial.print("  Peso: "); Serial.print(peso2); Serial.println("  ");
                    // Serial.print("Fase: ");
                    // Serial.println(fase);
                  }else{
                    fase = 4; 
                    Serial.println("------------------------------------------------------Cambio a Fase 4 por peso zero");
                    Serial.print(trama); Serial.print(" 694 - Fase: "); Serial.print(fase); Serial.print("  Estado: "); Serial.print(estado); Serial.print("  Peso: "); Serial.print(peso2); Serial.println("  ");                    
                  } 
            }
          }else{
            // EL PESO NO ES ESTABLE . ANALIZAMOS SI PASAMOS A FASE 4 
              if (peso2 <= 100){ //SI EL PESO BAJA A MENOS DE 100 GRAMOS PASAMOS A FASE 4
                fase = 4; 
                Serial.println("------------------------------------------------------Cambio a Fase 4 por peso inestable menor a 100 ");
                Serial.print(trama); Serial.print(" 702 - Fase: "); Serial.print(fase); Serial.print("  Estado: "); Serial.print(estado); Serial.print("  Peso: "); Serial.print(peso2); Serial.println("  ");
                } //PASAMOS A FASE 4 
          }
        ...
        case 4:
          ...
          Serial.print("xxxxxxxxxxxxxxxxxxxxxxxx Pes: "); Serial.println(pesoEstable);
          Serial.print("xxxxxxxxxxxxxxxxxxxxxxxx Estado: "); Serial.println(estadoEnvio);
          http.begin(envio);
          ...
          Serial.print(trama); Serial.print("  753 - Fase: "); Serial.print(fase); Serial.print("  Estado: "); Serial.print(estado); Serial.print("  Peso: "); Serial.print(peso2); Serial.println("  ");
          //PASAMOS A FASE
          PintaPantalla();
          break;
...
void CalculaTara(){
    ...
    //CALCULAR TARA 
    // ----------------------------------------------------------------------
    //Serial.print("CALCULAR TARA Peso: "); Serial.print(peso2); Serial.print(" Estable: "); Serial.print(estable);
    if ( tip == 1 ) { 
